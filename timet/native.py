...
class _Emitter:
    ...
    def _emit_print(self, arg: A.Expr, out: List[str]):
        ...
        if isinstance(ty, TFloat):
            raise NativeError(
                "native: printing Float values is not supported in v1 "
                "(matching Python's shortest-repr float formatting in C is "
                "a separate work item; print an Int or a Bool comparison)",
                span=_span(arg))
